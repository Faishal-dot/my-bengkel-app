<x-app-layout>
        ...
        <div class="max-w-3xl mx-auto animate-fadeIn">
            ...
            <div class="bg-white shadow-xl rounded-2xl border border-gray-100 p-8 hover:shadow-2xl transition-all duration-300">
                ...
                <form method="POST" action="{{ route('admin.mechanics.update', $mechanic) }}" class="space-y-6">
                    ...
                    @php
                        $inputs = [
                            ['label' => 'Nama Mekanik', 'icon' => 'user', 'name' => 'name', 'type' => 'text', 'required' => true, 'value' => $mechanic->name, 'max' => null],
                            // KTP: tipe number, max 16 digit
                            ['label' => 'Nomor KTP', 'icon' => 'id-card', 'name' => 'ktp', 'type' => 'number', 'required' => true, 'value' => $mechanic->ktp, 'max' => 16],
                            ['label' => 'Email Login', 'icon' => 'mail', 'name' => 'email', 'type' => 'email', 'required' => true, 'value' => $mechanic->user->email ?? '', 'max' => null],
                            ['label' => 'Password Baru', 'icon' => 'lock', 'name' => 'password', 'type' => 'password', 'required' => false, 'value' => '', 'max' => null],
                            ['label' => 'Konfirmasi Password Baru', 'icon' => 'shield-check', 'name' => 'password_confirmation', 'type' => 'password', 'required' => false, 'value' => '', 'max' => null],
                            // Telepon: tipe number, max 15 digit
                            ['label' => 'Telepon', 'icon' => 'phone', 'name' => 'phone', 'type' => 'number', 'required' => false, 'value' => $mechanic->phone, 'max' => 15],
                            ['label' => 'Alamat Rumah', 'icon' => 'map-pin', 'name' => 'address', 'type' => 'text', 'required' => false, 'value' => $mechanic->address, 'max' => null],
                        ];
                    @endphp
                    ...
                    <div class="transition-all duration-300 transform hover:scale-[1.015] animate-fadeInUp">
                        ...
                        >
                            @php
                                $currentSpec = old('specialization', $mechanic->specialization);
                            @endphp
                            <option value="" disabled>-- Pilih Spesialisasi --</option>
                            <option value="Mesin" {{ $currentSpec == 'Mesin' ? 'selected' : '' }}>Mekanik Mesin</option>
                            <option value="Kelistrikan" {{ $currentSpec == 'Kelistrikan' ? 'selected' : '' }}>Mekanik Kelistrikan</option>
                            <option value="Kaki-kaki" {{ $currentSpec == 'Kaki-kaki' ? 'selected' : '' }}>Mekanik Kaki-kaki</option>
                            <option value="AC Mobil" {{ $currentSpec == 'AC Mobil' ? 'selected' : '' }}>Spesialis AC</option>
                            <option value="Body Repair" {{ $currentSpec == 'Body Repair' ? 'selected' : '' }}>Body Repair</option>
                            <option value="Tune Up" {{ $currentSpec == 'Tune Up' ? 'selected' : '' }}>Service Berkala (Tune Up)</option>
                        </select>
                    </div>
    ...
    <style>
        ...
        /* Sembunyikan spinner pada input number */
