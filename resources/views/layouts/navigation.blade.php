                ...
                @foreach ($menus as $i => $menu)
                    ...
                    >
                        <a href="{{ route($menu['route']) }}" 
                           class="group flex items-center px-4 py-3 rounded-lg transition relative gap-3
                           {{ request()->routeIs($menu['route']) || request()->routeIs(str_replace('.index','.*',$menu['route']))
                                ? 'bg-white/20 backdrop-blur-md shadow-md font-semibold'
                                : 'hover:bg-white/10' }}">
                            @if (request()->routeIs($menu['route']) || request()->routeIs(str_replace('.index','.*',$menu['route'])))
                                <span class="absolute left-0 top-0 h-full w-1 bg-yellow-400 rounded-r-md"></span>
                            @endif
                            <i data-lucide="{{ $menu['icon'] }}" class="w-5 h-5 group-hover:scale-110 transition"></i>
